...
import { SCXMLBuilder, StateBuilder, TransitionBuilder, DataModelBuilder, DataBuilder } from '../builder';
...
describe('SCXML Validator', () => {
  ...
  describe('invalid documents', () => {
    ...
    it('should reject invalid initial state reference', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('Initial state "nonexistent" does not exist'))).toBe(true);
    });
    ...
    it('should reject invalid nested initial state reference', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('Initial state "nonexistent-child" does not exist'))).toBe(true);
    });
    ...
    it('should reject duplicate data ids', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('Duplicate data id: duplicate'))).toBe(true);
    });
  ...
  describe('complex validation scenarios', () => {
    ...
    it('should catch errors in deeply nested states', () => {
      ...
      expect(errors.some(e => e.message.includes('does not exist in state "level1"'))).toBe(true);
      expect(errors.some(e => e.message.includes('Duplicate state id: duplicate'))).toBe(true);
    });
  });
});