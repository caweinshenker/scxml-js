...
import { SCXMLBuilder, StateBuilder, TransitionBuilder, DataModelBuilder, DataBuilder, ParallelBuilder } from '../builder';
...
describe('SCXML Validator Comprehensive Tests', () => {
  ...
  describe('state hierarchy validation', () => {
    it('should validate complex nested hierarchies', () => {
      const doc = SCXMLBuilder.create()
        ...
        .addState(StateBuilder.create('root')
          .initial('level1a')
          .addState(StateBuilder.create('level1a')
            .initial('level2a')
    ...
    it('should detect circular initial references', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('does not exist in state'))).toBe(true);
    });

    it('should validate multiple levels of initial state references', () => {
      const doc = SCXMLBuilder.create()
        .initial('a')
        .addState(StateBuilder.create('a')
          .initial('b')
          .addState(StateBuilder.create('b')
            .initial('c')
  ...
  describe('transition validation', () => {
    ...
    it('should warn about transitions without event, condition, or target', () => {
      ...
      expect(errors.some(e =>
        e.severity === 'warning' &&
        e.message.includes('must have at least one of: event, cond, or target')
      )).toBe(true);
    });
    ...
    it('should reject invalid transition types', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('Invalid transition type'))).toBe(true);
    });
  });
  ...
  describe('parallel state validation', () => {
      ...
      const doc = SCXMLBuilder.create()
        ...
        .addParallel(ParallelBuilder.create('concurrent')
          ...
          .addState(StateBuilder.create('branch2')
            ...
            .build())
          .addState(StateBuilder.create('branch3')
            .initial('b3s1')
    ...
    it('should require at least two child states in parallel', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('must contain at least two child states'))).toBe(true);
    });

    it('should validate nested parallel regions', () => {
      const doc = SCXMLBuilder.create()
        .initial('outer')
        .addParallel(ParallelBuilder.create('outer')
          .addState(StateBuilder.create('branch1').build())
          .addParallel(ParallelBuilder.create('inner')
  ...
  describe('data model validation', () => {
    it('should validate complex data models', () => {
      const dataModel = DataModelBuilder.create()
        ...
        .addData(DataBuilder.create('object').content('{"key": "value"}').build())
    ...
    it('should detect duplicate data IDs across nested states', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('Duplicate data id: counter'))).toBe(true);
    });
    ...
    it('should validate data elements without IDs', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('Data element must have an id'))).toBe(true);
    });
  ...
  describe('history state validation', () => {
    ...
    it('should validate history state IDs', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('History element must have an id'))).toBe(true);
    });
    ...
    it('should validate history state types', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('must have a type attribute of "shallow" or "deep"'))).toBe(true);
    });
  ...
  describe('final state validation', () => {
    ...
    it('should require final state IDs', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('Final state must have an id'))).toBe(true);
    });
    ...
    it('should detect duplicate final state IDs', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('Duplicate state id: done'))).toBe(true);
    });
  });
  ...
  describe('cross-reference validation', () => {
    it('should validate complex state reference patterns', () => {
      const doc = SCXMLBuilder.create()
        .initial('main')
        .addState(StateBuilder.create('main')
          .initial('idle')
          ...
          .addState(StateBuilder.create('working')
            .initial('phase1')
            .addState(StateBuilder.create('phase1')
              .addTransition(TransitionBuilder.create()
                .event('next')
                .target('phase2')
                .build())
              .build())
            .addState(StateBuilder.create('phase2')
              .addTransition(TransitionBuilder.create()
                .event('complete')
                .target('../../done')
                .build())
  ...
  describe('edge case validations', () => {
    it('should handle empty SCXML document', () => {
      const doc = { scxml: {} };
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('must contain at least one state'))).toBe(true);
    });
    ...
    it('should handle null and undefined values gracefully', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('State must have an id'))).toBe(true);
    });
    ...
    it('should validate very large state machines', () => {
      ...
      for (let i = 0; i < 1000; i++) {
        builder.addState(StateBuilder.create(`state${i}`)
          .addTransition(TransitionBuilder.create()
            .event('next')
    ...
    it('should detect state ID collisions across different state types', () => {
      ...
      const errors = validator.validate(doc);
      expect(errors.some(e => e.message.includes('Duplicate state id: duplicate'))).toBe(true);
    });
  });
  ...
  describe('validation performance', () => {
    it('should validate complex documents efficiently', () => {
      ...
      for (let i = 0; i < 50; i++) {
        const nextState = StateBuilder.create(`level${i}`)
          .addTransition(TransitionBuilder.create()
            .event(`event${i}`)
            .target(`level${(i + 1) % 50}`)
            .build());
