...
describe("Enhanced SCXMLDocument Features", () => {
  ...
  describe("Fragment Insertion", () => {
    it("should insert valid SCXML fragments", () => {
      ...
      expect(
        document.scxml.state!.find((s) => s.id === "loading")
      ).toBeDefined();
  ...
  describe("Parallel State Conversion", () => {
    ...
    it("should create valid parallel structure when converting single state with enhancement", () => {
      ...
      testDoc.convertToParallelWithSubstates("target", [
        "region-a",
        "region-b",
      ]);
      ...
      expect(targetState?.parallel![0].state![1].id).toBe(
        "region-a_additional"
      );
    });
    ...
    it("should distribute multiple existing states across parallel regions", () => {
      ...
      testDoc.convertToParallelWithSubstates("parent", [
        "region-x",
        "region-y",
      ]);
  ...
  describe("Error Handling and Recovery", () => {
    ...
    it("should provide detailed error messages", () => {
      try {
        document.insertFragment(
          '<state><transition target="nowhere"/></state>'
        );
